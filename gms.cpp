...
int main(int argc, char* argv[])
{
    const char* keys =
            ...
            "{ l left        |  box.png                  | specify left (reference) image  }"
            "{ r right       |  box_in_scene.png                | specify right (query) image }"
            ...
            "{ withRotation  | true            | take rotation into account }"
    ...
    {
        Mat imgL_Origin = imread(cmd.get<String>("left"), IMREAD_GRAYSCALE);
        Mat imgR_Origin = imread(cmd.get<String>("right"), IMREAD_GRAYSCALE);
        Mat imgL,imgR;
        cv::resize(imgL_Origin, imgL, cv::Size(640, 480));
        cv::resize(imgR_Origin, imgR, cv::Size(640, 480));
        ...
        bool  isRatate=cmd.get<bool>("withRotation");

        matchGMS(imgR.size(), imgL.size(), kpCur, kpRef, matchesAll, matchesGMS, isRatate, cmd.get<bool>("withScale"));
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            cv::resize(originframe, frame, cv::Size(width, height));
